    ...
    with tab_dash:
        if is_live:
            ...
            with col_g:
                ...
                # Gráfico ajustado com limite superior dinâmico (resolve o problema de os valores irem para a direita e saírem de foco)
                gauge_max = max(40000, voc + (voc * 0.15))
                ...
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    gauge={
                        'axis':{'range':[19000, gauge_max],'tickwidth':1,'tickcolor':"#1E2D45",'tickfont':{'color':'#5A7090','size':10}},
                        ...
                        'steps':[
                            ...
                            {'range':[limiar_atual, gauge_max],  'color':"rgba(0,229,180,0.15)"}   # Fresca
                        ],
                        'threshold':{'line':{'color':cor_hex,'width':2},'thickness':0.8,'value':voc}
                    }
                ))
                # Mais margem nas laterais ajuda o número a não ficar descentrado quando é grande
                fig_gauge.update_layout(height=240, margin=dict(l=35,r=35,t=30,b=10), paper_bgcolor='rgba(0,0,0,0)')
            ...
            if df_hist_real.empty:
                ...
                if df_periodo.empty:
                    ...
                    <div class="kpi-row">
                        ...
                        <div class="kpi-card"><div class="kpi-num" style="color:#00E5B4;">{pct_ok}%</div><div class="kpi-lbl">Taxa Conformidade (Fresca)</div></div>
                    </div>
                    """, unsafe_allow_html=True)
